...
app.get('/console-messages', (req, res) => {
  res.json(consoleMessages);
});
...
const runCrawler = async () => {
  ...
  const scCities = ['Abdon Batista', 'Abelardo Luz', 'Agrolândia', 'Agronômica', 'Água Doce', 'Águas de Chapecó', 'Águas Frias',
    ...
    const processArticles = async (newspapers, articles) => {
        ...
        try {
            for (const article of articles) {
                try {
                    ...
                    await page.goto(article.url, {waitUntinl: 'domcontentloaded', timeout: 15000});
                    await page.waitForTimeout(5000);

                    const articleText = await page.evaluate(() => document.documentElement.outerHTML);
                    const articleDate = await page.evaluate(() => document.querySelector('.date, .published-date, .published, .post-date, .data-post, .data, .td-post-date, .jeg_meta_date, .post__data__published, .meta-date, .datas_interno, .entry-date')?.innerText);
                    ...
                    console.error(`Failed to process articles from newspaper: ${newspaper.name} - ${err}`);
                } finally {
                    await page.close();
                }
            ...
            for (const newspaper of newspapers) {
                try {
                    ...
                    await mainPage.goto(newspaper.urls, {waitUntil: 'domcontentloaded', timeout: 15000});
                    await mainPage.waitForTimeout(5000);

                    const articles = await mainPage.evaluate((keywords) => {
                    ...
                    const articleResults = await processArticles(newspaper, articles);
...
// Iniciar o servidor
app.listen(PORT, () => {
  logToConsole(`Servidor rodando em http://localhost:${PORT}`);
  runCrawler().catch(err => {
    logToConsole(`Erro ao executar o crawler: ${err}`);
  });
});
...
app.post('/start-crawler', async (req, res) => {
    try {
        await runCrawler();
        res.status(200).send('Crawler iniciado com sucesso');
    } catch (err) {
        console.error('Erro ao iniciar crawler: ', err);
        res.status(500).send('Erro ao inicializar crawler');
    }
});